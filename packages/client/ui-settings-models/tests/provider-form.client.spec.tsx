...
describe('endpoint interrogation', () => {
  ...
  it('adopts only the picked candidates, keeping a row the user already tuned', async () => {
    ...
    const modal = screen.getByRole('dialog', { name: en.fetchTitle })
...
describe('hand-declared providers', () => {
  ...
  it('scopes each card to fields a provider can actually own', async () => {
    ...
    expect(fields()).toEqual([en.customRoute, en.customDisplayName, en.baseUrl, en.customApi, en.useProxy, en.keyInput])
    cleanup()
    ...
    expect(fields()).toEqual([en.keyInput, en.baseUrl, en.useProxy])
    cleanup()
    ...
    openEditor('acme-gateway')
    expect(fields()).toEqual([en.keyInput, en.customDisplayName, en.baseUrl, en.customApi, en.useProxy])
  })
